...
export function Detail() {
  ...
  const available = (!demo.city || demo.categoryLive(demo.city, category)) && (!demo.city || demo.areaServed(demo.city))
  const saved = demo.favorites.includes(service.name)
      ...
      <main className="flex-1 overflow-y-auto px-6 pb-28 pt-6">
        ...
        <div className="mt-2 flex items-start justify-between gap-3">
          ...
          <button
            ...
            onClick={() => demo.toggleFavorite(service.name)}
          >
            {saved ? "Saved" : "Save"}
          </button>
        </div>
        ...
        {!available && (
          <p className="mt-4 rounded-xl bg-amber-50 px-3 py-2 text-xs font-semibold text-amber-800">
            {demo.city && !demo.areaServed(demo.city)
              ? "This visit is available in Rasayani."
              : `This category is paused in ${demo.city}.`}
          </p>
        )}
        <h2 className="section-title mt-7">Before the visit</h2>
        <p className="mt-2 text-sm text-slate-500">Someone should be home. Keep water and power on. Spare parts, if needed, are added on the job.</p>
        <h2 className="section-title mt-7">Not included</h2>
        <p className="mt-2 text-sm text-slate-500">Moving heavy furniture, and work outside the booked package.</p>
